                ...
                using (var _context = new PlayerBaseContext())
                {
                    foreach (var s in doneList)
                    {
                        ...
                        var wallLevel = playerBasePlacement.Where(l => l.BuildingTypeId == 3)
                            .Select(l => l.BuildingLevel).FirstOrDefault();
                        var playerScout = _context.PlayerScout.Where(l => l.UserId == s.TargetUserId)
                            .FirstOrDefault();
                        ...
                        var minVal = Math.Min(playerScout.SpyCount, s.AttackerSpyCount);
                        playerScout.SpyCount -= minVal;
                        s.AttackerDeadSpyCount = minVal;
                        s.DefenserDeadSpyCount = minVal;
                        ...
                        if (dbEnt != null)
                        {
                            ...
                            var ccc = SendScoutInfo(new BaseRequest<ScoutInfoDTO>()
                            {
                                Data = new ScoutInfoDTO()
                                {
                                    ...
                                    AttackerSpyCount = dbEnt.AttackerSpyCount,
                                    AttackerDeadSpyCount = dbEnt.AttackerDeadSpyCount,
                                    DefenserSpyCount = dbEnt.DefenserSpyCount,
                                    DefenserDeadSpyCount = dbEnt.DefenserDeadSpyCount,
